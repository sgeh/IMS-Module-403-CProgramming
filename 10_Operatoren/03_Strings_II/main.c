...
int main() {
	...
	char firstname[21] = { 0 };
	char lastname[21] = { 0 };
	char name[42] = { 0 }; // firstname (20) + lastname (20) + spacer (1) + string terminator (1)

	printf("\nBitte geben Sie Ihren Vor- und Nachnamen ein:\n");
	scanf("%20s %20s", &firstname, &lastname);

	strncat(name, firstname, strlen(firstname)); // Aufgabe c)
	strncat(name, " ", 1);  // Aufgabe c)
	strncat(name, lastname, strlen(lastname));  // Aufgabe c)

	if (strlen(firstname) >= 3 && strlen(lastname) >= 3 && strncmp(lastname, firstname, 21)) { // Aufgabe a) / b)
		for (bool doAbort = false; !doAbort; ) {
			printf("\nBitte Men\x81 ausw\x84hlen:\n");
			printf("\t1) Spaghetti\n");
			printf("\t2) Cannelloni\n");
		...
		printf("Die folgende Bestellung wurde aufgenommen f\x81r %s:\n", name);
		printf("\t%d x Spaghetti\n", spaghetti);
		printf("\t%d x Cannelloni\n", cannelloni);
		printf("\t%d x Pizza\n", pizza);
		return 0;
		...
		printf("Ung\x81litge Namenseingabe '%s'!\n", name);
		return 1;
	}
}
